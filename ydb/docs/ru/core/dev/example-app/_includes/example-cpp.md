...
```c++
auto connectionParams = NYdb::TDriverConfig()
    .SetEndpoint(endpoint)
    .SetDatabase(database)
    .SetAuthToken(GetEnv("YDB_TOKEN"));

NYdb::TDriver driver(connectionParams);
```
...
NYdb::NStatusHelpers::ThrowOnError(client.RetryQuerySync([](NYdb::NQuery::TSession session) {
    ...
    return session.ExecuteQuery(query,
        NYdb::NQuery::TTxControl::BeginTx(NYdb::NQuery::TTxSettings::SerializableRW()).CommitTx()).GetValueSync();
}));
```
...
NYdb::NStatusHelpers::ThrowOnError(client.RetryQuerySync([&resultSet](NYdb::NQuery::TSession session) {
    std::string query = R"(
...
std::optional<NYdb::TResultSet> resultSet;
NYdb::NStatusHelpers::ThrowOnError(client.RetryQuerySync([&resultSet](NYdb::NQuery::TSession session) {
    std::uint64_t seriesId = 2;
    std::uint64_t seasonId = 3;
    std::string query = R"(
...
NYdb::NStatusHelpers::ThrowOnError(client.RetryQuerySync([](NYdb::NQuery::TSession session) -> NYdb::TStatus {
    ...
    auto resultStreamQuery = session.StreamExecuteQuery(query, NYdb::NQuery::TTxControl::NoTx(), parameters).GetValueSync();
...
NYdb::NStatusHelpers::ThrowOnError(client.RetryQuerySync([&resultSet](NYdb::NQuery::TSession session) {
    ...
    auto params1 = NYdb::TParamsBuilder()
        .AddParam("$seriesId")
            .Uint64(seriesId)
            .Build()
        .AddParam("$seasonId")
...
Для продолжения работы в рамках текущей транзакции необходимо получить объект транзакции:

```c++
    // Get the active transaction object
    auto tx = resultValue.GetTransaction();

    // Processing the request result
    NYdb::TResultSetParser parser(resultValue.GetResultSet(0));
    parser.TryNextRow();
    auto date = parser.ColumnParser("from_date").GetOptionalUint64();

    // Perform some client logic on returned values
    auto userFunc = [] (const std::chrono::days& fromDate) {
        return fromDate + std::chrono::days(15);
    };

    auto fromDate = std::chrono::days(*date);
    auto toDate = userFunc(fromDate);
```
...
```c++
    ...
    auto params2 = NYdb::TParamsBuilder()
        .AddParam("$seriesId")
            .Uint64(seriesId)
            .Build()
        .AddParam("$fromDate")
    ...
    auto result2 = session.ExecuteQuery(
        query2,
        NYdb::NQuery::TTxControl::Tx(tx).CommitTx(),
...
while (parser.TryNextRow()) {
    auto airDate = std::chrono::days(*parser.ColumnParser("air_date").GetOptionalUint64());

    std::cout << "Episode " << parser.ColumnParser("episode_id").GetOptionalUint64()
        << ", Season: " << parser.ColumnParser("season_id").GetOptionalUint64()
        << ", Title: " << parser.ColumnParser("title").GetOptionalUtf8()
        << ", Air date: " << std::format("{:%a %b %d, %Y}", std::chrono::local_days(airDate))
        << std::endl;
}
```
...
NYdb::ThrowOnError(client.RetryQuerySync([](NYdb::NQuery::TSession session) -> NYdb::TStatus {
    auto airDate = TInstant::Now();
    auto beginResult = session.BeginTransaction(NYdb::NQuery::TTxSettings::SerializableRW()).GetValueSync();
